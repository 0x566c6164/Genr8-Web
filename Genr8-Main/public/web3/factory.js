// CONSTANTS
var contractAddress = '0xcf3f69f19cc609f9bd4fcae85b6e99676ee8db34';
...
function appendHTML(name, token, address, pointer) {
  var html = $(
    `<tr>
        <td><a href="/token/${pointer}">${pointer}</a></td>
        <td><a href="/token/${pointer}">${name}</a></td>
        <td>${token}</td>
        <td>${Math.floor(Math.random() * 10) + 1}%</td>
        <td>${Math.floor(Math.random() * 1000) - 100}%</td>
        <td>$${Math.floor(Math.random() * 1000000) + 1000}</td>
        <td>$${Math.floor(Math.random() * 10) + 1}</td>
